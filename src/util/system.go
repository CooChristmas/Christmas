...
func TemplateTextReplace(IPAddress string, ServerPort string, FileNameChristmas string, NameFolderExtesion string, SourceCodeName string, NameTemplate string) {
	...
	output = bytes.Replace(output, []byte("FileNameChristmas"), []byte(string(FileNameChristmas)), -1)
